...
def _build_queries() -> List[Dict[str, str]]:
    ...
    return [
        ...
        # ── Contract Dispute Resolution (Legal) ───────────────────────
        {
            "Query Id": "Q034",
            "Query": "What causal factors in legal contract-dispute calls lead to the customer retaining external counsel instead of accepting the proposed settlement?",
            "Query Category": "Causal Explanation",
            "System Output": "Customers retain external counsel when: (1) the agent uses ambiguous contractual language without citing specific clauses (discourse_clarification < 0.3, ATE=0.44), (2) settlement offers are perceived as significantly below the disputed amount (discourse_denial of full claim, ATE=0.39), and (3) the agent fails to acknowledge the customer's legal framing of the issue (discourse_apology < 0.1, ATE=0.27). The causal chain is: ambiguity + perceived_unfairness -> customer_distrust -> external_counsel_retention. Evidence: Turn 6 (Customer): 'Your terms are unclear and I need my attorney to review this.'",
            "Remarks": "Task 1 initial query; tests causal reasoning in Legal domain with contract-dispute intent."
        },
        {
            "Query Id": "Q035",
            "Query": "In those legal disputes, does providing a line-by-line contract walkthrough reduce the likelihood of counsel retention?",
            "Query Category": "Follow-up Reasoning",
            "System Output": "Yes. When agents provide a clause-by-clause walkthrough (discourse_clarification > 0.7 within Turns 3-6), the probability of external counsel retention drops from 0.71 to 0.34. The mechanism is: detailed clarification reduces perceived ambiguity, breaking the ambiguity -> distrust pathway. However, this effect is moderated by dispute amount—for claims exceeding $50,000, clarification alone is insufficient (retention remains at 0.58) because financial stakes independently drive legal escalation. Evidence: [Call 6120-4409, Turn 5] (Agent): 'Let me walk you through Section 3.2 which addresses your specific concern.'",
            "Remarks": "Task 2 follow-up to Query 34; tests moderation by financial magnitude in Legal domain."
        },
        # ── Student Enrollment Disputes (Education) ───────────────────
        {
            "Query Id": "Q036",
            "Query": "Why do university enrollment-dispute calls escalate more frequently when the student references financial-aid dependencies?",
            "Query Category": "Causal Explanation",
            "System Output": "Financial-aid references activate a compounding causal pathway: enrollment_error -> financial_aid_risk -> emotion_urgency (0.74) -> escalation. Students who mention aid dependencies show 2.4x higher escalation rates because unresolved enrollment errors threaten aid eligibility, housing, and visa status simultaneously. The agent's inability to provide cross-departmental resolution (discourse_delay=0.68 due to siloed systems) amplifies urgency. Evidence: Turn 4 (Student): 'If this isn't fixed today my financial aid gets revoked and I lose my housing.'",
            "Remarks": "Task 1 initial query; tests multi-dependency causal chains in Education domain."
        },
        {
            "Query Id": "Q037",
            "Query": "What counterfactual intervention in enrollment calls would most reduce escalation for students with financial-aid dependencies?",
            "Query Category": "Counterfactual Reasoning",
            "System Output": "Intervening with immediate cross-departmental conferencing (do(discourse_delay=0, cross_dept_access=1)) reduces escalation from 0.82 to 0.29. This is more effective than agent empathy alone (do(discourse_apology=0.9)) which only reduces escalation to 0.61, because the core driver is systemic urgency rather than emotional frustration. The optimal intervention combines: (1) real-time financial-aid hold placement to prevent automatic revocation, and (2) single-call resolution across registrar and aid offices.",
            "Remarks": "Task 2 follow-up to Query 36; tests counterfactual with systemic intervention in Education domain."
        },
        # ── Clinical Trial Adverse Event Reporting (Science) ──────────
        {
            "Query Id": "Q038",
            "Query": "What discourse patterns in clinical-trial adverse-event reporting calls predict whether the caller escalates to a regulatory complaint?",
            "Query Category": "Causal Explanation",
            "System Output": "Regulatory complaint escalation is driven by: (1) agent minimisation of reported symptoms (discourse_denial of severity, ATE=0.51), (2) failure to initiate mandatory reporting protocols within the call (discourse_delay > 0.6, ATE=0.43), and (3) caller perception that safety data is being suppressed (repeat requests for documentation, repetition=0.69). The causal chain is: minimisation + protocol_delay -> caller_distrust -> regulatory_complaint. Evidence: Turn 7 (Caller): 'I need confirmation that this event has been formally reported to the safety board.'",
            "Remarks": "Task 1 initial query; tests causal reasoning in Science/pharmaceutical domain with regulatory compliance implications."
        },
        {
            "Query Id": "Q039",
            "Query": "How do the reporting-call patterns differ when the adverse event is classified as serious versus non-serious?",
            "Query Category": "Follow-up Reasoning",
            "System Output": "Serious adverse events (SAEs) show fundamentally different dynamics: (1) caller emotion_urgency is 2.3x higher at Turn 1 (0.81 vs 0.35 for non-serious), (2) agents must follow expedited 24-hour reporting timelines, making discourse_delay impact 3.1x stronger (ATE=0.67 vs 0.22), (3) SAE callers are more likely to be healthcare professionals who demand protocol-specific language, so agent_response_quality has a stronger protective effect (ATE=-0.41 vs -0.18). Non-serious events escalate primarily through perceived dismissiveness rather than protocol urgency.",
            "Remarks": "Task 2 follow-up to Query 38; tests severity-stratified causal analysis in Science domain."
        },
        # ── Employee Grievance Handling (HR) ──────────────────────────
        {
            "Query Id": "Q040",
            "Query": "What causal factors in HR grievance calls predict whether an employee files a formal complaint with an external labour board?",
            "Query Category": "Causal Explanation",
            "System Output": "External filing is predicted by a three-stage cascade: (1) perceived procedural injustice—employee feels the internal process is biased (discourse_denial of evidence review, ATE=0.47), (2) retaliation concerns—employee references fear of consequences for raising the issue (emotion_urgency=0.66, ATE=0.38), (3) lack of timeline commitment from HR (discourse_delay > 0.5, ATE=0.31). The dominant causal path is: procedural_injustice -> distrust -> external_filing. Evidence: Turn 8 (Employee): 'If this isn't taken seriously internally, I have no choice but to go to the labour board.'",
            "Remarks": "Task 1 initial query; tests causal cascade in HR domain with grievance-handling intent."
        },
        {
            "Query Id": "Q041",
            "Query": "Does assigning a neutral third-party investigator during the call change the external-filing outcome in those HR grievance cases?",
            "Query Category": "Follow-up Reasoning",
            "System Output": "Yes, significantly. When the HR representative offers a neutral investigator within the first 5 turns (discourse_promise with neutrality language > 0.7), external filing probability drops from 0.68 to 0.27. The mechanism is: third-party assignment directly addresses the procedural_injustice -> distrust pathway by signalling impartiality. However, this intervention is ineffective when offered after Turn 8 (probability remains at 0.62), suggesting that distrust crystallises early. Evidence: [Call 7721-3305, Turn 4] (HR): 'I am assigning an independent investigator from outside your department to review this matter.'",
            "Remarks": "Task 2 follow-up to Query 40; tests temporal sensitivity of intervention in HR domain."
        },
        # ── Campaign Performance Disputes (Marketing) ─────────────────
        {
            "Query Id": "Q042",
            "Query": "What causal factors in marketing campaign-performance dispute calls lead clients to terminate their agency contract?",
            "Query Category": "Causal Explanation",
            "System Output": "Contract termination is driven by: (1) inability to attribute ROI to specific campaign actions (discourse_clarification < 0.3 on metrics questions, ATE=0.42), (2) repeated missed KPI targets referenced across multiple calls (repetition=0.71, ATE=0.38), (3) agent deflection to external market factors without acknowledging agency responsibility (discourse_denial of accountability, ATE=0.35). The causal chain is: attribution_failure + repeated_misses -> client_distrust -> contract_termination. Evidence: Turn 5 (Client): 'You keep blaming the algorithm but I need to see what your team actually did differently.'",
            "Remarks": "Task 1 initial query; tests causal reasoning in Marketing domain with performance-dispute intent."
        },
        {
            "Query Id": "Q043",
            "Query": "How do those marketing dispute patterns compare to the insurance claim-denial patterns we analysed earlier?",
            "Query Category": "Cross-Domain Comparison",
            "System Output": "Key parallels and differences: (1) Both domains show discourse_denial as a primary escalation driver (ATE=0.35 in marketing vs 0.47 in insurance), but the denial targets differ—marketing denies accountability while insurance denies coverage. (2) Marketing clients respond more strongly to data-driven clarification (ATE=-0.38) than insurance customers (ATE=-0.22), because marketing disputes are inherently metrics-based. (3) The temporal tipping point is earlier in marketing (Turns 4-5) versus insurance (Turns 6-8), reflecting shorter client patience in commercial relationships. Common factor: in both domains, denial without alternatives accelerates negative outcomes.",
            "Remarks": "Task 2 follow-up to Query 42 referencing Queries 22-23; tests cross-domain comparison between Marketing and Insurance."
        },
    ]
